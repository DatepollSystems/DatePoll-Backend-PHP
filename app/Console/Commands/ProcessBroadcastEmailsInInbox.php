...
class ProcessBroadcastEmailsInInbox extends Command {
      ...
      if ($userHasPermissionToSendBroadcasts != null) {
        if ($this->isBroadcastSubjectValid($subject)) {
          ...
          if ($this->containsSendToAllKeyword($subject)) {
            ...
            $this->broadcastRepository->create($subject, $textHtml, $mail->textPlain,
                                               $userHasPermissionToSendBroadcasts->user_id, array(), array(), true,
                                               array());
          } else {
            $groupsIds = array();
            ...
            foreach ($receiverStrings as $receiverGroupName) {
              ...
              if (!$foundSomething) {
                ...
                dispatch(new SendEmailJob(new BroadcastUnknownReceiver($receiverGroupName),
                                          array($mail->fromAddress)))->onQueue('high');
            ...
            $this->broadcastRepository->create($subject, $textHtml, $mail->textPlain,
                                               $userHasPermissionToSendBroadcasts->user_id, $groupsIds, array(), false,
                                               array());
          }
        } else {
          dispatch(new SendEmailJob(new BroadcastInvalidSubject(), array($mail->fromAddress)))->onQueue('high');
        }
      } else {
        Logging::info('processBroadcastEmails', 'Permission denied.');
        dispatch(new SendEmailJob(new BroadcastPermissionDenied(), array($mail->fromAddress)))->onQueue('high');
      }
  ...
  private function containsSendToAllKeyword(string $subject): bool {
    ...
    $keywords = array("Alle", "All", "Mitglieder", "Everyone");
